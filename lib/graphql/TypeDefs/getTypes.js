...
// 🔹 INQUIRY TYPE
// ========================
export const InquiryStatusEnum = new GraphQLEnumType({
  name: "InquiryStatus",
  values: {
    OPEN: { value: "OPEN" },
    CLOSED: { value: "CLOSED" }
...
export const InquiryMessageType = new GraphQLObjectType({
  name: "InquiryMessage",
  fields: () => ({
    id: { type: GraphQLInt },
    ...
    sender: { type: UserType } // Must exist in your system
  })
});
...
export const InquiryType = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    buyer: { type: UserType },
    seller: { type: UserType },
    property: { type: PropertyType },

    messages: { type: new GraphQLList(InquiryMessageType) }
  })
});
